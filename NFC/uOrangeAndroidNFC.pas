...
type
//  TOnNewNfcIntentEvent=procedure(Sender:TObject)
  ...
  public
    { Public declarations }
    NFCTagIdLabel_Text:String;
    InfoList:String;
    MessageSubscriptionID: Integer;
    FOnNewNfcIntentEvent:TNotifyEvent;
...
//procedure TOrangeAndroidNFC.FormCreate(Sender: TObject);
//var
//  ClassIntent: JIntent;
//begin
...
procedure TOrangeAndroidNFC.FormActivate(Sender: TObject);
  {$IFDEF ANDROID}
var
  Intent: JIntent;
  {$ENDIF}
...
//procedure TOrangeAndroidNFC.Button1Click(Sender: TObject);
//var
...
constructor TOrangeAndroidNFC.Create(AOwner: TComponent);
  {$IFDEF ANDROID}
var
  ClassIntent: JIntent;
  {$ENDIF}
...
destructor TOrangeAndroidNFC.Destroy;
begin

  inherited;
end;

procedure TOrangeAndroidNFC.EnableForegroundDispatch;
  {$IFDEF ANDROID}
var
  PEnv: PJniEnv;
...
//procedure TOrangeAndroidNFC.NfcCheckBoxChange(Sender: TObject);
//begin
//  if NfcAdapter <> nil then
//  begin
//    if NfcCheckBox_IsChecked then
...
function TOrangeAndroidNFC.ApplicationEventHandler(AAppEvent: TApplicationEvent;
  AContext: TObject): Boolean;
begin
  {$IFDEF ANDROID}
...
//            NfcCheckBox.Enabled := False;
        ...
        else if NfcCheckBox_IsChecked then
          EnableForegroundDispatch
      end;
    end;
    TApplicationEvent.WillBecomeInactive:
    begin
      if NfcCheckBox_IsChecked and (NfcAdapter <> nil) then
        NfcAdapter.disableForegroundDispatch(TAndroidHelper.Activity);
    end;
  ...
  Log.d('OrangeUI ', Self, 'ApplicationEventHandler', '-');  {$ENDIF}

end;

procedure TOrangeAndroidNFC.HandleIntentMessage(const Sender: TObject;
  const M: TMessage);
  {$IFDEF ANDROID}
var
  Intent: JIntent;
...
begin
  ...
  {$IFDEF ANDROID}
procedure TOrangeAndroidNFC.OnNewNfcIntent(Intent: JIntent);
  {$IFDEF ANDROID}
var
  TagParcel: JParcelable;
  Tag: JTag;
...
begin
  {$IFDEF ANDROID}
  Log.d('OrangeUI TOrangeAndroidNFC.OnNewIntent');
...
//  InfoList.Items.Clear;
  InfoList:='';
  NFCTagIdLabel_Text := HandleNfcTag(Tag,
...
//          InfoList.Items.Add(Strings[I]);
          InfoList:=InfoList+Strings[I]+#13#10;
        end;
      finally
        Strings.Free;
      end;
...
//  InfoList.Visible := True;
  ...
  if Assigned(FOnNewNfcIntentEvent) then
  begin
    FOnNewNfcIntentEvent(Self);
  end;
  {$ENDIF}
end;
  {$ENDIF}
...
function TOrangeAndroidNFC.ReadTagBlock(ABlockIndex: Integer;
...
function TOrangeAndroidNFC.WriteTagBlock(ABlockIndex: Integer;ABlockDataStr:String): Boolean;
  {$IFDEF ANDROID}
var
  TagParcel: JParcelable;
  Tag: JTag;
...
//procedure TOrangeAndroidNFC.TagWriteButtonClick(Sender: TObject);
//var
//  TagParcel: JParcelable;
//  Tag: JTag;
//  Intent: JIntent;
