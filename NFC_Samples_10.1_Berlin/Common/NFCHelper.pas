...
begin
  ...
  begin
    ...
    try
      Log.d('OrangeUI Listing tag techs');
...
var
  JATag: JNfcA;
begin
  Log.d('OrangeUI Found tech type A');
...
var
  JBTag: JNfcB;
begin
  Log.d('OrangeUI Found tech type B');
  // Technical details to be found at http://www.waazaa.org/download/fcd-14443-3.pdf
...
var
  JFTag: JNfcF;
begin
  Log.d('OrangeUI Found tech type F');
...
var
  JVTag: JNfcV;
begin
  Log.d('OrangeUI Found tech type V');
...
var
  ...
  I: Integer;
begin
  Log.d('OrangeUI Found tech type NDEF');
  JNDefTag := TJNdef.JavaClass.get(Tag);
  JNDefTagType := JNDefTag.getType;
  Log.d('OrangeUI NDEF type: ' + JStringToString(JNDefTagType));
...
var
  JIsoTag: JIsoDep;
begin
  Log.d('OrangeUI Found tech type ISO Dep');
...
//  NDefRecords.Items[1] := TJNdefRecord.JavaClass.createUri(StringToJString('http://blong.com'));
  ...
  begin
   ...
   // Let's try to format the Tag in NDEF
    nForm:=TJNdefFormatable.JavaClass.get(Tag);
    if nForm<>nil then
    begin
      nForm.connect;
      nForm.format(NDefMsg);
      nForm.close;
      Exit;
    end
    else
