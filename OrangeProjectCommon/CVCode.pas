...
type
  TTwoBytes=Array [1..2] of Byte;

  function GBtoBIG5(value: TBytes): TBytes;
  function BIG5toGB(value: TBytes): TBytes;
  function isGB(value: TTwoBytes): Boolean;
  function isBIG5(value: TTwoBytes): Boolean;
  function Cht2Chs(value:String):String;
  function Chs2Cht(value:String):String;
...
var
  ...
  ABIG5Bytes:TBytes;
begin
  AGB2312Bytes:=TEncoding.GetEncoding('BIG5').GetBytes(value);

  //繁体转换成简体
  ABIG5Bytes:=BIG5toGB(AGB2312Bytes);

  Result:=TEncoding.GetEncoding('GB2312').GetString(ABIG5Bytes);
...
var
  ...
  ABIG5Bytes:TBytes;
begin
  AGB2312Bytes:=TEncoding.GetEncoding('GB2312').GetBytes(value);

  //简体转换成繁体
  ABIG5Bytes:=GBtoBIG5(AGB2312Bytes);

  Result:=TEncoding.GetEncoding('BIG5').GetString(ABIG5Bytes);
end;

function GBOffset(value: TTwoBytes): integer;
begin
...
function BIG5Offset(value: TTwoBytes): integer;
begin
  Result := -1;
  if length(value) >= 2 then
  begin
...
//        output := output + WordToString(GBOrder[offset]);
        output[idx] := Hi(GBOrder[offset]) ;
        output[idx+1] := Lo(GBOrder[offset]) ;
...
//        output := output + WordToString(BIG5Order[offset]);
        output[idx] := Hi(BIG5Order[offset]) ;
        output[idx+1] := Lo(BIG5Order[offset]) ;
        inc(idx);
      end
      else
      begin
