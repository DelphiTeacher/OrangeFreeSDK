...
type
  ...
  protected
    ...
    procedure NotifyAnimationBegan; virtual;
    procedure NotifyAnimationEnded; virtual;
  public
    constructor Create;
    destructor Destroy; override;
    /// <summary>Destroys current picker instance and ensures that this does not happen during the animation process.</summary>
    procedure Release;
    procedure Show;
    procedure Hide;
    { IDialogActions }
...
begin
  FPopupDateTimePicker.Release;
  FPopupDateTimePicker := nil;
  inherited Destroy;
end;
...
procedure TPopupDialog.NotifyAnimationBegan;
begin
  FIsAnimating := True;
end;

procedure TPopupDialog.NotifyAnimationEnded;
begin
  FIsAnimating := False;
  if FIsReleasing then
    Destroy;
end;
...
begin
  ...
  try
    ...
    FUICloseButton.setTitle(StrToNSStr(Translate(SPickerCancel)));
    ...
    FUIDoneButton.setTitle(StrToNSStr(Translate(SPickerDone)));
...
begin
  ...
  FOnShow := nil;
  FOnHide := nil;
  FUIContainerView.layer.removeAllAnimations;
...
procedure TPopupDialog.Release;
begin
  if FIsAnimating then
    // If the picker is being animated, we mark it for releasing in the end of animation
    FIsReleasing := True
  else
    Destroy;
end;
...
begin
  ...
  begin
    ...
    try
      TUIView.OCClass.setAnimationDuration(AnimationDuration);
      FUIContainerView.setFrame(EndFrame);
      TUIView.OCClass.setAnimationDidStopSelector(sel_getUid('Shown'));
      NotifyAnimationBegan;
    finally
      TUIView.OCClass.commitAnimations;
    end;
...
{+++>}    FUIToolBar.setBarStyle(UIBarStyleDefault)
{+++>}  else
...
{+++>}var PickerView: UIView; // fix by Aone
begin
  // We should Create instance of UIDatePicker before creating main container.
  // Because We use UIViewPicker for determinate finally size of picker container
  // (Toolbar + Picker)
...
{+++>}  // from child's views. Without it, when we rotate picker, we can see a problem
{+++>}  // with bad picker rotating and scaling (in slow motion).
{+++>}  // [0] subview - is a UIPickerView
{+++>}  if FDateTimePicker.subviews.count > 0 then                                                                                                            
{+++>}  begin                                                                                                                                                 
{+++>}    PickerView := TUIView.Wrap(FDateTimePicker.subviews.objectAtIndex(0));                                                                              
{+++>}    PickerView.setAutoresizingMask(UIViewAutoresizingFlexibleWidth or
{+++>}      UIViewAutoresizingFlexibleLeftMargin or UIViewAutoresizingFlexibleRightMargin);
{+++>}  end;
{+++>}  FDateTimePicker.setAutoresizingMask(UIViewAutoresizingFlexibleWidth or
{+++>}    UIViewAutoresizingFlexibleLeftMargin or UIViewAutoresizingFlexibleRightMargin);
{+++>}  FUIContainerView.addSubview(FDateTimePicker);
{+++>}end;
end;
