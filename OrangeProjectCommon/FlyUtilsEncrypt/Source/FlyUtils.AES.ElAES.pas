...
procedure EncryptAESStreamECB(Source: TStream; Count: FixedUInt;
...
procedure EncryptAESStreamCBC(Source: TStream; Count: FixedUInt;
...
procedure DecryptAESStreamECB(Source: TStream; Count: FixedUInt;
  const Key: TAESKey128; Dest: TStream;
  APaddingMode: TPaddingMode = TPaddingMode.pmPKCS5or7RandomPadding;
  UniocdeLEMode: Boolean = False;
  OnProcessProc: TOnProcessProc = nil; ProcessProc: TProcessProc = nil); overload;
...
procedure DecryptAESStreamCBC(Source: TStream; Count: FixedUInt;
  const Key: TAESKey128; const InitVector: TAESBuffer; Dest: TStream;
  APaddingMode: TPaddingMode = TPaddingMode.pmPKCS5or7RandomPadding;
  UniocdeLEMode: Boolean = False;
  OnProcessProc: TOnProcessProc = nil; ProcessProc: TProcessProc = nil); overload;
...
const
  ...
  ForwardTable: array [0..255] of FixedUInt = (
  ...
  LastForwardTable: array [0..255] of FixedUInt = (
  ...
  InverseTable: array [0..255] of FixedUInt = (
  ...
  LastInverseTable: array [0..255] of FixedUInt = (
...
begin
  ...
  // finalizing
  PFixedUInt(@OutBuf[0])^ := T0[0]; PFixedUInt(@OutBuf[4])^ := T0[1];
  PFixedUInt(@OutBuf[8])^ := T0[2]; PFixedUInt(@OutBuf[12])^ := T0[3];
end;
  ...
  begin
    ...
    begin
      ...
      if (APaddingMode = TPaddingMode.pmPKCS5or7RandomPadding) and
        (Count >= 3) then
      begin
        TempIn[Done + 1 + Random(Count - 2)] := Random(255);
      end;
    end;
    EncryptAES(TempIn, ExpandedKey, TempOut);
    Done := Dest.Write(TempOut, SizeOf(TempOut));
