...
begin
  ...
  if TThread.GetTickCount - AtomicCmpExchange(LastTotalMemoryUsedByTexturesLog, 0, 0) > 1000 then begin // every 1 sec
    ...
    Log.d('TALTexture TotalMemoryUsedByTextures: ' + FormatFloat('#.##', AtomicCmpExchange(TotalMemoryUsedByTextures, 0, 0) / 1000000) +' MB');
  end;
  if TALTextureAccessPrivate(self).FBits <> nil then
    Log.d('TALTexture.Assign Bits: ' + FormatFloat('#.##',(Width * Height * BytesPerPixel) / 1000) +' kB');
  {$ENDIF}
end;
