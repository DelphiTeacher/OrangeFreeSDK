//http://bbs.2ccc.com/topic.asp?topicid=529138
...
begin
  ...
  case TypeInfo^.Kind of
    ...
    tkSet:
      begin
        Result := ctx.GetType(TypeInfo).AsSet.TypeSize;
...
{$ENDIF CPU}
    tkSet:
      Result := ctx.GetType(TypeInfo).AsSet.TypeSize > SizeOf(NativeInt);
  else
    Result := False;
  end;
end;
...
begin
  if FData.FTypeInfo = nil then
    ...
    case FData.FTypeInfo^.Kind of
      ...
      tkSet:
        Exit(ctx.GetType(FData.FTypeInfo).AsSet.GetTypeSize);
      tkFloat:
...
{$IFDEF MACOS}
  ...
  begin
    ...
    if (AResultType <> nil) and (AResultType^.Kind = tkRecord) then
    begin
...
var
  ...
  begin
    ...
    else if (Arg.GetTypeKind = tkRecord) or (Arg.GetTypeKind = tkArray) then
    begin
      src := Arg.GetReferenceToRawData;
      while datasize > 0 do
      begin
...
begin
  ...
  try
    if not IsStatic then
      if IsClassMethod then
        Info.AddParameter(TypeInfo(TClass))
      else
        Info.AddParameter(Parent.Handle);

    for p in GetParameters do
      if p.ParamType = nil then
        Info.AddParameter(nil, True)
      else
      begin
...
begin
  Result := System.TypInfo.SizeOfSet(PTypeInfo(Handle));
end;

function TRttiSetType.GetByteOffset: Integer;
begin
  Result := System.TypInfo.ByteOffsetOfSet(PTypeInfo(Handle));
end;
...
begin
  ...
  case TypeInfo^.Kind of
    ...
    tkSet:
      begin
        Result := ctx.GetType(TypeInfo).AsSet.GetTypeSize;
...
begin
  ...
  begin
    ...
    else if IsManaged(FReturnType) or ((FReturnType^.Kind = tkRecord) and (GetTypeData(FReturnType)^.RecSize > 16)) then
    begin
      FResultLoc := TParamLoc.Create(FReturnType, True);
      PutArg(FResultLoc);
    end;
...
{$IF defined(DEBUG) or (not defined(MSWINDOWS))}
