(* ****************************************************** *)
(*　　　　　　　　　　　　　　　　　　　　　　　　　　　　*)
(*　　　　修改：爱吃猪头肉 & Flying Wang 2016-04-20　　　 *)
(*　　　　　　　　上面的版权声明请不要移除。　　　　　　　*)
(*　　　　　　　　　　　　　　　　　　　　　　　　　　　　*)
(*　　　    　　　　　禁止发布到城通网盘。　　　　  　　　*)
(*　　　　　　　　　　　　　　　　　　　　　　　　　　　　*)
...
//Fix or Add By 爱吃猪头肉。
procedure CallInUIThreadMayWaitFinishing(AMethod: TMethodCallback; ToWait: Boolean); overload;
procedure CallInUIThreadMayWaitFinishing(AMethod: TCallBack; ToWait: Boolean); overload;
procedure CallInNotUIThreadMayWaitFinishing(AMethod: TMethodCallback; ToWait: Boolean); overload;
procedure CallInNotUIThreadMayWaitFinishing(AMethod: TCallBack; ToWait: Boolean); overload;
//Fix or Add By 爱吃猪头肉。
...
//在某些时候使用 CallInUIThreadAndWaitFinishing 会导致切换 APP 卡死。
//大部分设备 MianThread <> UIThread 但是 不需要调用 CallInUIThread
//前提是 Application 完成初始化。
//10.2.x and up MianThread = UIThread
//Fix or Add By 爱吃猪头肉。
var
...
begin
  Result := False;
  if UIThreadID = 0 then
  begin
    Runnable := TRunnable.Create(procedure()begin
      UIThreadID := TThread.CurrentThread.ThreadID;
    end);
    ActiveJavaRunnables.Add(Runnable);
    Runnable.Start;
  end;
  if TThread.CurrentThread.ThreadID <> UIThreadID then exit;
  Result := True;
end;
...
//Fix or Add By 爱吃猪头肉。
procedure CallInUIThreadMayWaitFinishing(AMethod: TMethodCallback; ToWait: Boolean);
begin
  if ToWait then
    CallInUIThreadAndWaitFinishing(AMethod)
  else
    CallInUIThread(AMethod);
end;

//Fix or Add By 爱吃猪头肉。
procedure CallInUIThreadMayWaitFinishing(AMethod: TCallBack; ToWait: Boolean);
begin
  if ToWait then
    CallInUIThreadAndWaitFinishing(AMethod)
  else
...
begin
  try
    ...
    if SupportTimer then begin
      FTimerService.DestroyTimer(FTimerHandle);
      //fixed by Chee Wee Chua && added by kngstr
      //https://quality.embarcadero.com/browse/RSP-24446
      FTimerHandle := 0;
    end;
  end;
end;
