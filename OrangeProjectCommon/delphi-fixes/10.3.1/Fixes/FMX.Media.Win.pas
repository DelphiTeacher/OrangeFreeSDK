    ...
    begin
      ...
      begin
        ...
        if FVMRWindowlessControl7 <> nil then
          FVMRWindowlessControl7.SetAspectRatioMode(VMR_ARMODE_LETTER_BOX);
      end
      else
      begin
        if FVMRWindowlessControl9 <> nil then
          FVMRWindowlessControl9.SetAspectRatioMode(VMR9ARMode_None);
        if FVMRWindowlessControl7 <> nil then
          FVMRWindowlessControl7.SetAspectRatioMode(VMR_ARMODE_NONE);
      end;
      if Stretch then
        R := TRect.Create(0, 0, Control.BoundsRect.Round.Width, Control.BoundsRect.Round.Height)
      else
...
//Fix By Flying Wang and 爱吃猪头肉.
procedure TWindowsMedia.SetStretch(const Value: Boolean);
begin
  FStretch := Value;
  UpdateMediaFromControl;
end;

procedure TWindowsMedia.SetProportional(const Value: Boolean);
begin
  FProportional := Value;
  UpdateMediaFromControl;
end;

function TWindowsMedia.GetStretch: Boolean;
begin
  Result := FStretch;
end;

function TWindowsMedia.GetProportional: Boolean;
begin
  Result := FProportional;
end;

function TWindowsMedia.GetRealWebMediaObject: IUnknown;
begin
  if FVMRWindowlessControl9 <> nil then
  begin
