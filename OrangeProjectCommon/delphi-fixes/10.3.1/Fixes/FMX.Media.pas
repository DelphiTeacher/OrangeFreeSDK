...
//useage:
//MediaPlayer1.Proportional := True; //安卓和 IOS 下总是 True
//MediaPlayer1.Stretch := True; //IOS 下 总是 True

// GetRealWebMediaObject 是新增的跨平台的属性。
// (MediaPlayer1.GetRealWebMediaObject as IBaseFilter).FindPin;
...
//Fix By Flying Wang and 爱吃猪头肉.
  protected
    procedure SetStretch(const Value: Boolean); virtual; abstract;
    procedure SetProportional(const Value: Boolean); virtual; abstract;
    function GetStretch: Boolean; virtual; abstract;
    function GetProportional: Boolean; virtual; abstract;
  public
    //Fix or Add By 爱吃猪头肉
    function GetRealWebMediaObject: IUnknown; virtual; abstract;
  published
...
//Fix By Flying Wang and 爱吃猪头肉.
  ...
  //Fix or Add By 爱吃猪头肉
    function GetRealWebMediaObject: IUnknown;
  published
...
//Fix By Flying Wang and 爱吃猪头肉.
procedure TMediaPlayer.SetStretch(const Value: Boolean);
begin
  if FMedia <> nil then
    FMedia.Stretch := Value;
  FStretch := Value;
end;

procedure TMediaPlayer.SetProportional(const Value: Boolean);
begin
  if FMedia <> nil then
    FMedia.Proportional := Value;
  FProportional := Value;
end;

function TMediaPlayer.GetStretch: Boolean;
begin
  if FMedia <> nil then
    Result := FMedia.Stretch
  else
    Result := FStretch;
end;

function TMediaPlayer.GetProportional: Boolean;
begin
  if FMedia <> nil then
    Result := FMedia.Proportional
  else
    Result := FProportional;
end;

function TMediaPlayer.GetRealWebMediaObject: IUnknown;
begin
  if FMedia <> nil then
    Result := FMedia.GetRealWebMediaObject
  else
    Result := nil;
end;
