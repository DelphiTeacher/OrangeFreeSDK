//http://www.cnblogs.com/onechen/p/6079080.html
//https://quality.embarcadero.com/browse/RSP-16590
//http://www.cnblogs.com/onechen/p/6209965.html
//增加 [重庆]老新手(371889755) 找来的 修改了listview造成越界 的问题。
...
//fix or add by flyingwang.
var
  /// <remarks>
  ///   是否使用单字符折行
  /// </remarks>
  GlobalUseSingleCharacterBreakMode: Boolean = False;
  /// <remarks>
  ///   是否避开标点在行首的情况
  /// </remarks>
  GlobalLineFirstNotPunctuation: Boolean = False;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ClipBounds := TRectF.Create(TopLeft.X, TopLeft.Y, MaxSize.X + TopLeft.X, MaxSize.Y + TopLeft.Y);
            ClipPadding.Top := Max(R.Top, ClipBounds.Top) - R.Top;
            ClipPadding.Left := Max(R.Left, ClipBounds.Left) - R.Left;
            ClipPadding.Right := R.Right - Min(R.Right, ClipBounds.Right);
            ClipPadding.Bottom := R.Bottom - Min(R.Bottom, ClipBounds.Bottom);

            R.Top := R.Top + ClipPadding.Top;
            R.Bottom := R.Bottom - ClipPadding.Bottom;
            R.Left := R.Left + ClipPadding.Left;
            R.Right := R.Right - ClipPadding.Right;
          ...
          if not SrcR.IsEmpty and not R.IsEmpty then
          begin
            // Draw
...
{--->
        if (FFrame.Last.TopLeft.Y + FFrame.Last.Height) > MaxSize.Y then
          if FFrame.Last.TopLeft.Y > MaxSize.Y then
<---}
{+++>}
...
{<+++}
          begin
            FFrame.Height := FFrame.Height - FFrame.Last.Height;
            ...
            while FFrame[I].Count > 0 do
              ...
              begin
                ...
                begin
                  ...
                  //fix or add by flying wang.
                  //fix by xubzhlin
                  //当TListItemText.Align = TListItemAlign.Trailing
                  //listview 在改变 DeleteLayout Width
                  //造成的 Run.Chars 越界(iOS有这个问题,Windwos正常，其他未测试)
                  //不明什么原因(可能是由于 TListItemText 的宽度为负数造成的)
                  //有兴趣的可以自己测试一下
                  //<---------------
                  //while (X < 0) do
                  //---------------->
                  //<+++++++++++++++
                  while (X < 0) and (Run.Length > 0) do
                  //++++++++++++++++>
                  begin
                    Run.Clip;
...
begin
  ...
  LineIndex := 0;
//fix or add by flying wang.
{+++>}var h: Single := Padding.Top; // 2017/01/11 修正显示省略字符 by Aone
...
//https://quality.embarcadero.com/browse/RSP-16590
                if GlobalUseSingleCharacterBreakMode then
                begin
                  if GlobalLineFirstNotPunctuation then
                  begin
                  ...
                  begin
                    while (WordBeginIndex > LRun.StartIndex) do // 单字符折行
                      Dec(WordBeginIndex);
                  end;
                end
                else
                begin
                  ...
                  begin
                    while (WordBeginIndex > LRun.StartIndex) and not (Text.Chars[WordBeginIndex - 1].GetUnicodeCategory in
                      [TUnicodeCategory.ucSpaceSeparator,
                      TUnicodeCategory.ucOtherLetter]) do
                      Dec(WordBeginIndex);
                  end;
                end;
////fix or add by flying wang. end.
...
{+++>}              // 2017.01.13 修正避开首字标点 by Aone
                    if ((not GlobalLineFirstNotPunctuation) and Text.Chars[CurrentPos - 1].IsLetterOrDigit) or (
                      GlobalLineFirstNotPunctuation and (
                      Text.Chars[CurrentPos - 1].IsLetterOrDigit or
                       Text.Chars[CurrentPos - 1].IsPunctuation or
                      (Text.Chars[CurrentPos - 1].GetUnicodeCategory = TUnicodeCategory.ucOtherLetter)
                      )) then
{<+++}
                    begin
