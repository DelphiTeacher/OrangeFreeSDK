...
//Fix By Flying Wang and 爱吃猪头肉 and [龟山]阿卍(1467948783);
procedure TCommonWebBrowserService.EvaluateJavaScript(const JavaScript: string;
...
begin
  LJavaScript := StrToNSStr(JavaScript);
  AResult := NSStrToStr(FWebView.stringByEvaluatingJavaScriptFromString(LJavaScript)); //fix by [成都]kevin(546060808)
...
//Fix or Add By 爱吃猪头肉
function TCommonWebBrowserService.GetRealWebBrowserObject: IUnknown;
begin
  Result := FWebView;
end;

function TCommonWebBrowserService.GetRedirectionKey: string;
begin
  Result := FRedirectionKey;
end;

procedure TCommonWebBrowserService.SetRedirectionKey(const Value: string);
begin
...
begin
  NewURL := URL;
  //Fix 重定向关键字，回电，刷新，不 Load NewURL;
