...
//Fix 重定向关键字
  if FWebBrowser <> nil then
  begin
    FURLSetter.SetURL(NSStrToStr(willPerformClientRedirectToURL.absoluteString));
    FWebBrowser.ShouldStartLoading(FWebBrowser.URL);
//    if (FWebBrowser <> nil) and
//      (FWebBrowser.RedirectionKey <> '') and
//      (Pos(FWebBrowser.RedirectionKey, TNetEncoding.URL.Decode(FWebBrowser.URL)) <> 0) then
//    begin
//      Exit;
//    end;
  end;
