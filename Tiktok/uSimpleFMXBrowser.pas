...
type
  TSimpleFMXBrowserFrm = class(TForm)
    ...
    procedure FMXChromium1LoadEnd(Sender: TObject; const browser: ICefBrowser;
  ...
  public
    FIsLoaded:Boolean;
    FOnWebBrowserLoadEnd:TWebBrowserLoadEndEvent;
...
procedure TSimpleFMXBrowserFrm.tmrLoadEndTimer(Sender: TObject);
begin
  tmrLoadEnd.Enabled:=False;
  if Assigned(FOnWebBrowserLoadEnd) then
  begin
    FOnWebBrowserLoadEnd(Self,Self.FMXChromium1.Browser.MainFrame.Url);
  end;
...
procedure TSimpleFMXBrowserFrm.FMXChromium1LoadEnd(Sender: TObject;
  const browser: ICefBrowser; const frame: ICefFrame; httpStatusCode: Integer);
begin
  if Self.FMXChromium1.Browser.MainFrame.Url <> 'about:blank' then
  begin
    FIsLoaded:=True;
    uBaseLog.HandleException(nil,'====================°æ‰Ø¿¿∆˜º”‘ÿÕÍ≥…£∫' + Self.FMXChromium1.Browser.MainFrame.Url + '°ø====================');

    TThread.Synchronize(nil,procedure
    begin

//      edtUrl.Text:=Self.Chromium1.Browser.MainFrame.Url;
