...
type
  ...
  // fix +++++++++++
  TWebBrowserJavascriptCallback = procedure(ASender: TObject; const Value: string) of object;
  TWebBrowserOpenFileChooser = procedure(ASender: TObject; Code: Integer) of object;
  // +++++++++++++++
...
{ IFMXWBService }
  ...
  private
    ...
    // fix +++++++++
    FOnJavascriptCallback: TWebBrowserJavascriptCallback;
    FOnOpenFileChooser: TWebBrowserOpenFileChooser;
    // +++++++++++
    function GetURL: string;
    function GetCanGoBack: Boolean;
    function GetCanGoForward: Boolean;
  ...
  public
    ...
    //fix  ++++++++++
    property OnJavascriptCallback: TWebBrowserJavascriptCallback read FOnJavascriptCallback write FOnJavascriptCallback;
    property OnOpenFileChooser: TWebBrowserOpenFileChooser read FOnOpenFileChooser write FOnOpenFileChooser;
    // +++++++++++++
  end;
...
// fix ++++++++++++
procedure TCustomWebBrowser.JavascriptCallback(Value: string);
begin
  if Assigned(FOnJavascriptCallback) then
    FOnJavascriptCallback(Self, Value);
end;

procedure TCustomWebBrowser.OpenFileChooser(Code: Integer);
begin
  if Assigned(FOnOpenFileChooser) then
    FOnOpenFileChooser(Self, Code);
end;
